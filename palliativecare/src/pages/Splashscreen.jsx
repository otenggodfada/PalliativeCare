...
// Adjust based on your logic to determine the first visit
  ...
  // Handle navigation based on authentication status and first visit
  useEffect(() => {
    if ( user) {
      const timer = setTimeout(() => {
        navigate('/home');
        setisLoading(true) // Navigate to Home if the user is logged in
      }, 3000); // Set timeout for 3 seconds

      return () => clearTimeout(timer); // Cleanup the timer
    } else if (!user) {
    const timers = setTimeout(()=>{  navigate('/login'); setisLoading(true)}, 3000)
  ...
  const handleFullClick = () => {
    if (isFirstVisit) {
      setIsFirstVisit(false); // Example logic to update the first visit status
    }
  };

  return (
    <div>
      {isLoading? (<div></div>): <div className="flex flex-col items-center justify-end min-h-screen ">
    <div className="p-2 flex flex-col   items-center">
      ...
      <div className=" text-center text-fuchsia-950  text-3xl font-bold  font-['Inter']">
        We are always ready for <br />
        your health care
      </div>
      <div className=" text-center">
        <span className="text-fuchsia-950 text-2x1 font-semibold font-['Inter']">
          Contacting a palliative care now is
          <br />
        </span>
        <span className="text-mypink  font-semibold font-['Inter']">
          {" "}
          easy and quick!
        </span>
      </div>
    ...
    <div className="w-full h-full pt-20 bg-white rounded-tl-full rounded-tr-full  shadow-blue-400 shadow-2xl flex-col justify-end items-center inline-flex">
      ...
      <div  onClick={handleFullClick} className="w-12 h-12 px-2 py-2  animate-pulse  origin-center absolute  bottom-16  rotate-[38.01deg] bg-mypink rounded justify-center items-center inline-flex">
        <svg
